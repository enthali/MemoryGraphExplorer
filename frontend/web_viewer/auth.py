...
class AuthManager:
    ...
    def validate_request(self, required_permission: str = 'read') -> Tuple[bool, Optional[str], Optional[Dict]]:
        """
        ...
        if not self.auth_enabled:
            return True, None, {'name': 'Auth Disabled', 'permissions': ['read', 'write', 'admin']}
        ...
        if not api_key:
            return False, 'Missing API key. Provide X-API-Key header or Authorization: Bearer token', None
        ...
        if required_permission not in key_info['permissions'] and 'admin' not in key_info['permissions']:
            return False, f'Insufficient permissions. Required: {required_permission}', None
