...
class MCPHTTPClient:
        ...
        try:
            ...
            init_request = {
                ...
                "params": {
                    ...
                    "capabilities": {"tools": {}},
                    "clientInfo": {"name": "web-server-client", "version": "1.0.0"},
                },
            }
            ...
            self.connected = True
            print(
                f"✅ Connected to StreamableHTTP MCP Server (session: {self.session_id})"
            )
    ...
    def _send_notification(self, notification: Dict[str, Any]):
        ...
        response = self.session.post(
            self.base_url, json=notification, headers=headers, timeout=10
        )
    ...
    def _parse_sse_response(self, sse_text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to parse JSON from SSE data: {data_line}, error: {e}"
            )
    ...
    def call_tool(self, tool_name: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            request = {
                ...
                "id": self._next_id(),
                "method": "tools/call",
                "params": {"name": mcp_tool_name, "arguments": mcp_params},
            }
            ...
            response = self.session.post(
                self.base_url, json=request, headers=headers, timeout=30
            )
...
def get_full_graph():
    ...
    try:
        ...
        print(
            f"✅ Served full knowledge graph: {len(graph_data.get('entities', []))} entities, {len(graph_data.get('relations', []))} relations"
        )
...
def search_graph():
    ...
    try:
        ...
        print(
            f"✅ Search '{query}': found {len(search_results.get('entities', []))} entities, {len(search_results.get('relations', []))} relations"
        )
...
def get_entity():
    ...
    try:
        ...
        open_result = mcp_client.call_tool(
            "memory_open_nodes", {"names": [entity_name]}
        )
        entity = None
        ...
        relations_result = mcp_client.call_tool(
            "memory_get_node_relations", {"node_name": entity_name}
        )
        relations = relations_result.get("outgoing", []) + relations_result.get(
            "incoming", []
        )
        ...
        entity_data = {"entity": entity, "relations": relations}

        print(
            f"✅ Served entity details for '{entity_name}' with {len(relations)} relations"
        )
...
def get_node_relations():
    ...
    try:
        relations_data = mcp_client.call_tool(
            "memory_get_node_relations", {"node_name": entity_name}
        )

        # Add some metadata
        total_relations = len(relations_data.get("outgoing", [])) + len(
            relations_data.get("incoming", [])
        )
        ...
        print(
            f"✅ Enhanced API: Served relations for '{entity_name}' - {total_relations} total relations"
        )
...
def mcp_proxy(subpath=""):
    ...
    internal_url = (
        os.environ.get("MCP_PROXY_INTERNAL_URL")
        or getattr(mcp_client, "base_url", None)
    ...
    try:
        ...
        return (
            jsonify(
                {"error": f"MCP proxy error: {str(e)}", "internal_url": internal_url}
            ),
            500,
        )
...
def run_server(host="localhost", port=8080, debug=False):
    ...
    print(
        f"🚀 Starting Knowledge Graph Web Server (StreamableHTTP) on http://{host}:{port}"
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Knowledge Graph Web Server (StreamableHTTP)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="MCP server URL (default: http://localhost:3001/mcp)",
    )
    ...
    try:
        ...
        print(f"❌ Failed to connect to MCP server: {e}")
        print(
            "💡 Make sure the StreamableHTTP MCP Memory Server is running on port 3001"
        )
